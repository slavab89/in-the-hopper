...
import { HopperOptions } from './types';
...
const onRequestWrapper =
  ({ fieldInterpreters, resolver, handler, timestamps, immediate, ignore }: HopperOptions): RequestWrapper =>
...
const koaMiddlewareWrapper = (triggerHandler: Function) => async (ctx: Koa.Context, next: Function) => {
  triggerHandler(ctx.res)(ctx);
  await next();
};

const expressMiddlewareWrapper = (triggerHandler: Function) => (req: Express.Request, res: Express.Response, next: Function) => {
...
export default (
  { middlewareCreator, type }: { middlewareCreator: (opts: HopperOptions) => Function; type?: string },
